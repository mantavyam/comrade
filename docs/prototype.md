...
Flutter's `lib/` is the core; group by feature (e.g., auth, news) for scalability, with cross-cutting concerns (e.g., constants) at top-level. This matches your flows: auth screens, news cards, profile stats.

```
comrade-app/
├── frontend/
│   ├── android/                # Android-specific configs (auto-generated)
│   ├── ios/                    # iOS-specific configs (auto-generated)
│   ├── lib/
│   │   ├── main.dart           # Entry point: App widget, routes, theme setup
│   │   ├── routes.dart         # Centralized navigation (e.g., named routes for screens)
│   │   ├── constants/          # Global constants (e.g., colors.dart, strings.dart, api_endpoints.dart)
│   │   │   ├── colors.dart
│   │   │   ├── strings.dart    # Text like "Welcome Comrade"
│   │   │   └── api_endpoints.dart  # Backend URLs
│   │   ├── models/             # Data models (e.g., NewsItem, UserProfile, QuizQuestion)
│   │   │   ├── news_item.dart
│   │   │   ├── user_profile.dart
│   │   │   └── quiz_question.dart
│   │   ├── services/           # Business logic (e.g., API calls, Firebase auth, RSS handling proxy via backend)
│   │   │   ├── auth_service.dart  # Firebase auth wrappers
│   │   │   ├── news_service.dart  # HTTP to backend for news
│   │   │   └── quiz_service.dart
│   │   ├── providers/          # State management (e.g., BLoC for auth, news state)
│   │   │   ├── auth_provider.dart
│   │   │   └── news_provider.dart
│   │   ├── screens/            # Full screens (pages) grouped by flow
│   │   │   ├── splash_screen.dart
│   │   │   ├── get_started_screen.dart
│   │   │   ├── auth/           # Subfolder for auth-related screens
│   │   │   │   ├── login_screen.dart
│   │   │   │   ├── signup_name_screen.dart
│   │   │   │   ├── signup_phone_screen.dart
│   │   │   │   ├── otp_screen.dart
│   │   │   │   └── forget_password_flow.dart  # Could be a widget or separate screens
│   │   │   ├── home_screen.dart  # News tab default
│   │   │   ├── editorials_screen.dart
│   │   │   ├── bookmarks_screen.dart
│   │   │   ├── stories_quiz_screen.dart
│   │   │   ├── profile_screen.dart
│   │   │   └── notifications_screen.dart
│   │   └── widgets/            # Reusable UI components (e.g., cards, buttons)
│   │       ├── news_card.dart  # Swipeable card
│   │       ├── expanded_news_detail.dart
│   │       ├── bookmark_category.dart
│   │       ├── streak_icons.dart  # Energy bolts for profile
│   │       ├── custom_button.dart  # White rounded buttons
│   │       └── quiz_question_widget.dart
│   ├── assets/                 # Images, GIFs, fonts
│   │   ├── images/             # Logos, hero images, military GIFs
│   │   └── fonts/              # Custom fonts if needed
│   ├── pubspec.yaml            # Dependencies (firebase_auth, http, flutter_swiper, etc.)
│   └── test/                   # Unit/integration tests (e.g., widget tests for cards)
└── README.md                   # Project overview, setup instructions
```

- **Why this structure?** Feature-based (e.g., auth subfolder) for easy location. Widgets are reusable (e.g., news_card in home and editorials). Services handle async ops, keeping screens clean.

#### Backend (Python with FastAPI) Structure
FastAPI favors router-based organization for APIs. Group by endpoints (e.g., news, auth). Use Pydantic for models.

```
comrade-app/
├── backend/
│   ├── main.py                 # App entry: FastAPI instance, include routers
│   ├── requirements.txt        # Dependencies (fastapi, uvicorn, feedparser, nltk, firebase-admin)
│   ├── routers/                # API endpoints grouped
│   │   ├── news_router.py      # /news, /editorials (RSS parsing)
│   │   ├── quiz_router.py      # /quiz (generate from news)
│   │   ├── stories_router.py   # /stories (static hero data)
│   │   └── auth_router.py      # Token validation (Firebase)
│   ├── models/                 # DB models (Pydantic schemas)
│   │   ├── news.py             # NewsItem schema
│   │   ├── user.py             # UserProfile
│   │   └── quiz.py
│   ├── utils/                  # Helper functions
│   │   ├── rss_parser.py       # Feedparser logic for PIB/Hindu/etc.
│   │   ├── quiz_generator.py   # NLTK-based MCQ creation
│   │   └── firebase_utils.py   # Firestore interactions
│   ├── config.py               # Env vars (e.g., Firebase creds, API keys)
│   ├── schemas/                # Optional: SQL schemas if using SQLite
│   └── tests/                  # API tests (e.g., pytest for endpoints)
└── .env                        # Secrets (Firebase keys, etc.)
```

- **Why this structure?** Router per feature for modularity (e.g., add /bookmarks later). Utils for reusable code like RSS fetching (scheduled daily via APScheduler in main.py).
